...
public class HopperCommands {
  ...
  public static Command setHopperModeTimed(
      Hopper hopper, HopperConstants.RunMode mode, double timeSeconds) {
    HopperConstants.RunMode oldMode = currentMode;
    return setHopperMode(hopper, mode)
        .andThen(Commands.waitSeconds(timeSeconds))
        .andThen(setHopperMode(hopper, oldMode));
  }
}
