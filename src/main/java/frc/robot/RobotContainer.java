...
public class RobotContainer {
    ...
    private void configureDriverControllerBindings(CommandXboxController xbox) {
        ...
        xbox.rightTrigger()
                .whileTrue(aimDrive)
                .onTrue(rumbleController(operatorController, 0.05, RumbleType.kLeftRumble)
                        .until(() -> LaunchCommands.isDriveAtLaunchGoal(drive))
                        .andThen(rumbleController(operatorController, 0.1, RumbleType.kRightRumble)
                                .withTimeout(0.1)));
    ...
    private void configureOperatorControllerBindings(CommandXboxController xbox) {
        ...
        // Intake button (hold)
        xbox.leftTrigger()
                .whileTrue(intake.run(() -> intake.setMode(IntakeRunMode.INTAKING))
                        .finallyDo(() -> intake.setMode(IntakeRunMode.UP))
                        .withName("Dump Intake"))
                .whileTrue(hopper.run(() -> hopper.setMode(HopperRunMode.IDLE))
        ...
        xbox.leftBumper()
                .debounce(0.1)
                .whileTrue(intake.run(() -> intake.setMode(IntakeRunMode.OUTTAKING))
                        .finallyDo(() -> intake.setMode(IntakeRunMode.UP))
                        .withName("Dump Intake"))
                .whileTrue(hopper.run(() -> hopper.setMode(HopperRunMode.REVERSE))
                        .finallyDo(() -> hopper.setMode(HopperRunMode.STOPPED))
                        .withName("Dump Hopper"));

        // Deploy intake tap button
        xbox.leftStick()
                .onTrue(intake.runOnce(() -> intake.setModeNoWheels(IntakeRunMode.INTAKING))
                        .withName("Deploy intake"));

        // Retract intake tap button
        xbox.rightStick()
                .onTrue(intake.runOnce(() -> intake.setModeNoWheels(IntakeRunMode.UP))
        ...
        xbox.rightTrigger()
                .whileTrue(launcher.runOnce(launcher::start)
        ...
        xbox.a()
                .and(manualButton.negate())
                .whileTrue(hopper.run(() -> hopper.setMode(HopperRunMode.PREP_SHOT))
                        .finallyDo(() -> hopper.setMode(HopperRunMode.STOPPED))
                        .withName("Reverse Hopper"));

        // --- MANUAL LAUNCH MODE CONTROLS ---

        final Trigger anyManualModeLetterButton =
                xbox.a().or(xbox.b()).or(xbox.x()).or(xbox.y());
        ...
        xbox.povRight().and(manualButton.or(isManualMode)).onTrue(manualLaunchControl.incrementHoodCommand(+0.025));
        xbox.povLeft().and(manualButton.or(isManualMode)).onTrue(manualLaunchControl.incrementHoodCommand(-0.025));
    ...
    private SendableChooser<Command> createSendableChooser() {
        ...
        var chooser = Constants.DEVELOPMENT_MODE ? AutoBuilder.buildAutoChooser() : new SendableChooser<Command>();
