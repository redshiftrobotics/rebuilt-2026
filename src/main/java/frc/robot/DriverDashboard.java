...
public class DriverDashboard {
    ...
    public static void updateDashboard() {
        ...
        SmartDashboard.putNumber(
                "Speed MPH",
                Units.metersPerSecondToMilesPerHour(Math.hypot(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond)));

        SmartDashboard.putBoolean("Has Vision", hasVisionEstimateDebounce.calculate(hasVisionEstimate.getAsBoolean()));
            ...
            public void initSendable(SendableBuilder builder) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    builder.addDoubleProperty(
                            moduleNames[i] + " Angle", () -> wheelStatesSupplier.get()[index].angle.getRadians(), null);
                    builder.addDoubleProperty(
                ...
                builder.addDoubleProperty(
                        "Robot Angle",
                        () -> AllianceMirrorUtil.apply(poseSupplier.get().getRotation())
                                .getRadians(),
                        null);
            }
        });
