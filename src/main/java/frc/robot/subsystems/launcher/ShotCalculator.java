...
public class ShotCalculator {
  ...
  public static Translation2d adjustedHubPosition(
      Translation2d hubPosition, Translation2d robotVelocityMetersPerSecond, Function<Double, Rotation2d> pitchProvider) {
    Translation2d adjustedHubPosition = hubPosition;
    for (int i = 0; i < 5; i++) {
      Rotation2d pitch = pitchProvider.apply(adjustedHubPosition.getNorm());
      ShotParameters parameters = new ShotParameters(calculateVelocity(adjustedHubPosition.getNorm(), pitch), pitch);
