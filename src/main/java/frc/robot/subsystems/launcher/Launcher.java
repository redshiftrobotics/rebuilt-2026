...
public class Launcher extends SubsystemBase {
    ...
    public final TunablePID flywheelPID = new TunablePID(getName() + "/PID", ChannelConstants.FLYWHEEL_PID);
    ...
    public static TunableNumber DEBOUNCE_TIME_AT_GOAL_TELE = new TunableNumber("Launcher/DebounceTimeAtGoalTele", 0.25);
    ...
    public void configure(Supplier<Pose2d> robotPoseSupplier, Supplier<ChassisSpeeds> robotVelocitySupplier) {
    ...
    public void periodic() {
        ...
        for (int i = 0; i < channelIOs.size(); i++) {
            ...
            Logger.processInputs(getName() + "/Channel" + channelIOs.get(i).getName(), channelInputs.get(i));
        ...
        switch (mode) {
            case INTERPOLATION: {
                ...
                setRunningDesiredState(new LauncherState(parameters.wheelRadPerSec(), parameters.hoodPosition()));
                ...
                SmartDashboard.putNumber(
                        ...
                        Units.metersToInches(parameters.distance()
            ...
            case MATHEMATICAL: {
                ...
                setRunningDesiredState(new LauncherState(parameters.getWheelRadPerSec(), parameters.getHoodPosition()));
                robotYaw = parameters.yaw();
                break;
            }
            case MANUAL: {
                // fallback to simple aiming option
                robotYaw = LaunchCalculator.getStationaryAimedPose(robotPose.getTranslation())
                        .getRotation();
        ...
        atGoalDebounced = DriverStation.isAutonomous()
                ? autoAtGoalDebouncer.calculate(atGoal)
                : teleAtGoalDebouncer.calculate(atGoal);

        Logger.recordOutput(getName() + "/desiredRobotPose", new Pose2d(robotPose.getTranslation(), robotYaw));

        SmartDashboard.putNumber(
                "LauncherTuning/MeasuredWheelRadPerSec", getMeasuredState().wheelRadPerSec());
        SmartDashboard.putNumber("LauncherTuning/DesiredWheelRadPerSec", runningDesiredState.wheelRadPerSec());
        SmartDashboard.putNumber("LauncherTuning/DesiredHoodPosition", runningDesiredState.hoodPosition());
