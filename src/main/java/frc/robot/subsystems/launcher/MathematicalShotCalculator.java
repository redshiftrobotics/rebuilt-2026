...
public class MathematicalShotCalculator {
    ...
    public static MathematicalShotParameters calculateAndSetShot(
        ...
        Function<Double, Rotation2d> pitchCalculator =
                isHoodAdjustable ? MathematicalShotCalculator::calculatePitch : d -> HoodConstants.FIXED_LAUNCH_ANGLE;
        ...
        Translation2d hubTranslation = MathematicalShotCalculator.adjustedHubPosition(
                hubLocation,
        ...
        LinearVelocity ballLinearVelocity = MathematicalShotCalculator.calculateVelocity(distance, runningHoodPitch);
    ...
    public static LinearVelocity calculateVelocity(double distanceMeters, Rotation2d pitch) {
        ...
        double velocity = distanceMeters
                * Math.sqrt(9.81
                        / (2
    ...
    public static double getHoodPositionFromAngle(Rotation2d angle) {
        ...
        Translation2d hoodPosition = new Translation2d(r.times(angle.getCos()), r.times(angle.getSin()));

        double lengthMeters = hoodPosition.getDistance(HoodConstants.LAUNCHER_AXLE_TO_ACTUATOR);

        double position = (lengthMeters - HoodConstants.ACTUATOR_LENGTH_MIN.in(Meters))
    ...
    public static Rotation2d getAngleFromHoodPosition(double position) {
        double actuatorLength =
                position * HoodConstants.ACTUATOR_EXTENSION.in(Meters) + HoodConstants.ACTUATOR_LENGTH_MIN.in(Meters);
        ...
        Rotation2d relativeAngle = new Rotation2d(2 * Math.PI
                - Math.acos(
                        (radius * radius + actuatorDistance * actuatorDistance - (actuatorLength * actuatorDistance))
