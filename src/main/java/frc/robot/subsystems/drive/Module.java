...
public class Module {
    ...
    private static final TunablePID drivePID = moduleGains.pid("Drive_PID", ModuleConstants.DRIVE_FEEDBACK);
    private static final TunableFF driveFF = moduleGains.ff("Drive_FF", ModuleConstants.DRIVE_FEEDFORWARD);
    private static final TunablePID turnKp = moduleGains.pid("Turn_PID", ModuleConstants.TURN_FEEDBACK);
    private static final TunableFF turnFF = moduleGains.ff("Turn_FF", ModuleConstants.TURN_FEEDFORWARD);
    ...
    public Module(ModuleIO io, Translation2d distanceFromCenter) {
        ...
        driveDisconnectedAlert =
                new Alert(String.format("Disconnected drive motor on module %s.", toString()), AlertType.kError);
        turnDisconnectedAlert =
                new Alert(String.format("Disconnected turn motor on module %s.", toString()), AlertType.kError);
        turnAbsoluteEncoderDisconnectedAlert = new Alert(
                String.format("Disconnected absolute turn encoder on module %s.", toString()), AlertType.kError);
    }
    ...
    public void setSpeeds(SwerveModuleState state) {
        ...
        boolean nearlyAligned = MathUtil.isNear(
                angleRadians, moduleCurrentAngle.getRadians(), Units.degreesToRadians(turnAlignmentTolerance.get()));
