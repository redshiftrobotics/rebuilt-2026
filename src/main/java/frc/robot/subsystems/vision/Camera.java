...
public class Camera {
    ...
    private static final TunableNumber xyStdDevCoefficient = group.number("xyStdDevCoefficient", 0.075);
    private static final TunableNumber thetaStdDevCoefficient = group.number("thetaStdDevCoefficient", 0.085);

    private static final TunableNumber zHeightToleranceMeters = group.number("zHeightToleranceMeters", 0.6);
    private static final TunableNumber pitchAndRollToleranceDegrees = group.number("pitchToleranceDegrees", 10.0);
    ...
    public Camera(CameraIO io, Supplier<Pose2d> robotPoseSupplier) {
        ...
        this.missingCameraAlert = new Alert(
    ...
    private ProcessedEstimatedRobotPose processVision(EstimatedRobotPose estimate) {
        ...
        if (aprilTagFieldLayout == null) {
            ...
            status =
                    getStatus(estimate.estimatedPose, estimate.targetsUsed, estimate.strategy, robotPoseSupplier.get());
        ...
        return new ProcessedEstimatedRobotPose(
                estimate.estimatedPose, estimate.timestampSeconds, status, standardDeviations, tagPositionsOnField);
    }
    ...
    private Matrix<N3, N1> getStandardDeviations(List<Pose3d> tagPoses, Pose2d lastRobotPose) {
        ...
        DoubleSummaryStatistics distancesToTags = tagPoses.stream()
                .mapToDouble(
                        (estimatedPose) -> estimatedPose.getTranslation().getDistance(lastRobotPose3d.getTranslation()))
                .summaryStatistics();
        ...
        double standardDeviation = distancesToTags.getCount() > 0
    ...
    private VisionResultStatus getStatus(
        ...
        if (!usedTargets.stream()
                .allMatch(tag ->
                        aprilTagFieldLayout.getTagPose(tag.getFiducialId()).isPresent())) {
        ...
        double pitchAndRollToleranceValueRadians = Units.degreesToRadians(pitchAndRollToleranceDegrees.get());
        if (!MathUtil.isNear(0, estimatedRobotPose.getRotation().getX(), pitchAndRollToleranceValueRadians)
                && !MathUtil.isNear(0, estimatedRobotPose.getRotation().getY(), pitchAndRollToleranceValueRadians)) {
        ...
        if (filterBasedOnLastPose
                && estimatedRobotPose.toPose2d().getTranslation().getDistance(lastRobotPose.getTranslation())
